...
const moduleLogger = createModuleLogger('pay-url');
...
async function handlePayUrl(req, res) {
  ...
  try {
    ...
    if (!validateCrmPayToken(crmPayToken, CRM_PAY_TOKEN_EXPECTED)) {
      moduleLogger.warn('Invalid crm-pay-token', { 
        requestId, 
    ...
    moduleLogger.info('Creating payment URL', {
      requestId,
      orderId: order_id,
      clientId: client_id,
      amount,
    ...
    moduleLogger.info('Payment URL generated successfully', {
      requestId,
      orderId: order_id,
    ...
    const duration = timer.end();
    moduleLogger.error('Error generating payment url', {
      requestId,
      error: err?.message,
      stack: err?.stack,
      duration: `${duration}ms`
...
module.exports = createModuleServerlessHandler(handlePayUrl, 'pay-url');
